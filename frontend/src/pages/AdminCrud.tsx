...
const cleanPayload = (
  ...
  return Object.fromEntries(
    Object.entries(payload)
      .map(([key, value]) => [
        key,
        numericKeys.includes(key) ? toOptionalNumber(value) : value,
      ])
...
const AdminCrud = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return searchableData
      .filter((entry: { item: any; searchText: string }) =>
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (activeTab === "jobs") {
        ...
        return editingItem
          ? updateJob(editingItem.id, payload)
          : createJob(payload);
      }
      ...
      if (activeTab === "candidates") {
        ...
        return editingItem
          ? updateCandidate(editingItem.id, payload)
    ...
    onSuccess: async () => {
      setFeedback(
        `${tabLabels[activeTab].slice(0, -1) || tabLabels[activeTab]} saved`,
      );
      setDrawerOpen(false);
      setEditingItem(null);
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async (_, item) => {
      if (activeTab === "jobs" && selectedJobId === item.id)
        setSelectedJobId(null);
      if (activeTab === "candidates" && selectedCandidateId === item.id)
        setSelectedCandidateId(null);
      setFeedback(
        `${tabLabels[activeTab].slice(0, -1) || tabLabels[activeTab]} deleted`,
      );
      await invalidateWorkflow();
      window.setTimeout(() => setFeedback(null), 2500);
    },
  ...
  const handleDelete = useCallback(
    (item: any) => {
      const label = activeTab === "jobs" ? item.title : item.name;
      if (
        window.confirm(
          `Delete ${label}? Related matches and decision records may be removed.`,
        )
      ) {
        deleteItem(item);
      }
  ...
  const renderedRows = useMemo(() => {
    if (activeTab === "jobs") {
      return paginatedData.map((job: any) => (
        <tr
          key={job.id}
          className="bg-neu-surface hover:shadow-neu-inner transition-all"
        >
          <td className="px-5 py-4 font-black text-neu-primary">#{job.id}</td>
          <td className="px-5 py-4 font-bold text-neu-text">
            {job.title || "Untitled job"}
          </td>
          <td className="px-5 py-4 text-neu-text/60">
            {job.company_name || "-"}
          </td>
          <td className="px-5 py-4 text-neu-text/60">{job.location || "-"}</td>
          <td
            className="max-w-xs px-5 py-4 text-neu-text/60 truncate"
            title={job.skills}
          >
            {job.skills || "-"}
          </td>
    ...
    if (activeTab === "candidates") {
      return paginatedData.map((candidate: any) => (
        <tr
          ...
          className="bg-neu-surface hover:shadow-neu-inner transition-all"
        >
          <td className="px-5 py-4 font-black text-neu-primary">
            #{candidate.id}
          </td>
          <td className="px-5 py-4 font-bold text-neu-text">
  ...
  const renderField = (field: FieldConfig) => {
    const baseClass =
      "w-full bg-neu-surface shadow-neu-inner rounded-xl px-4 py-3 text-xs font-bold text-neu-text outline-none disabled:opacity-50";
    ...
    <div className="flex h-full min-w-0 flex-col space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="text-3xl font-black tracking-tight text-neu-text">
            Data management
          </h2>
          <p className="mt-1 max-w-2xl text-sm leading-6 text-neu-text/45">
            Maintain jobs and candidates with a layout that stays usable on
            tablets and smaller laptops.
          </p>
        </div>
        {feedback && (
          <div className="rounded-xl bg-neu-surface px-5 py-3 text-sm font-semibold text-neu-success shadow-neu-inner">
            {feedback}
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-neu-surface shadow-neu rounded-2xl p-4">
            ...
            <button
              ...
              className={`rounded-xl px-5 py-3 text-sm font-semibold transition-all ${
                activeTab === tab
                  ...
                  : "bg-neu-surface text-neu-text shadow-neu-sm"
              }`}
            >
              {tabLabels[tab]}
            </button>
        ...
        <div className="flex w-full flex-col gap-3 lg:w-auto lg:flex-row lg:items-center">
          <div className="text-sm font-medium text-neu-text/45 lg:text-right">
            {isSearchPending
              ? "Filtering..."
              : `${filteredData.length.toLocaleString()} records`}
          </div>
          <div className="flex w-full items-center gap-3 rounded-xl bg-neu-surface px-4 py-3 shadow-neu-inner lg:min-w-[320px]">
            <Search className="w-4 h-4 text-neu-primary" />
            <input
              value={searchTerm}
              onChange={(event) => setSearchTerm(event.target.value)}
              placeholder={`Search ${tabLabels[activeTab].toLowerCase()}`}
              className="w-full bg-transparent text-sm font-medium text-neu-text outline-none placeholder:text-neu-text/30"
            />
          </div>
          <button
            onClick={openCreate}
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-600 px-5 py-3 text-sm font-semibold text-white shadow-lg transition-all hover:bg-teal-700 active:scale-95"
          >
      ...
      <div className="bg-neu-surface shadow-neu rounded-2xl overflow-hidden flex-1 min-h-0">
        {isLoading ? (
          <div className="p-10 text-neu-text/50 font-bold animate-pulse">
            Loading data...
          </div>
        ) : isError ? (
          <div className="p-10 text-neu-danger font-bold">
            Failed to load CRUD data.
          </div>
        ) : (
          <div className="h-full flex flex-col">
            <div className="table-shell overflow-auto flex-1 min-h-0">
              <table className="w-full text-left text-sm">
                <thead className="sticky top-0 z-10 bg-neu-surface text-[11px] font-semibold tracking-[0.14em] text-neu-text/40 uppercase shadow-sm">
                  <tr>
                    {headers.map((header) => (
                      <th
                        key={header}
                        className={`px-5 py-4 ${header === "Actions" ? "text-right" : ""}`}
                      >
                        {header}
                      </th>
                    ))}
              ...
              {!filteredData.length && (
                <div className="p-12 text-center text-neu-text/40 font-bold">
                  No records found.
                </div>
              )}
            </div>

            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-300/60 bg-neu-surface px-5 py-3">
              <div className="text-sm text-neu-text/45">
                Showing {filteredData.length ? pageStart + 1 : 0}-
                {Math.min(pageStart + PAGE_SIZE, filteredData.length)} of{" "}
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={safePage <= 1}
                  className="rounded-xl bg-neu-surface px-4 py-2 text-sm font-semibold text-neu-text shadow-neu-sm disabled:opacity-40"
                >
                  Prev
                </button>
                <span className="min-w-24 text-center text-sm text-neu-text/45">
      ...
      {isDrawerOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/25 flex justify-end">
          <form
            onSubmit={submitForm}
            className="w-full max-w-xl h-full bg-neu-secondary shadow-2xl p-6 overflow-y-auto"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {fields[activeTab].map((field) => (
                ...
                >
                  <span className="block text-[11px] font-semibold uppercase tracking-[0.14em] text-neu-text/50">
                    {field.label}
                    {field.required ? " *" : ""}
                  </span>
                  {renderField(field)}
            ...
            <div className="flex gap-3 justify-end mt-8">
              <button
                type="button"
                onClick={() => setDrawerOpen(false)}
                className="rounded-xl bg-neu-surface px-5 py-3 text-sm font-semibold text-neu-text shadow-neu-sm"
              >
                Cancel
              </button>
              <button
                ...
                className="rounded-xl bg-teal-600 px-6 py-3 text-sm font-semibold text-white shadow-lg hover:bg-teal-700 disabled:opacity-50"
              >
